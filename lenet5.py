...
def lenet_mod(x, n_classes, grayscale=False):
    ...
    conv_concat = tf.concat([conv1_flat, conv2_flat], 1)

    # Layer 3: Fully Connected. Input = 400+1176. Output = 240.
    w3 = tf.Variable(tf.truncated_normal([400+1176,240], mean = mu, stddev = sigma))
    b3 = tf.Variable(tf.zeros([240]))
    fc1 = tf.add(tf.matmul(conv_concat, w3), b3)
    ...
    w4 = tf.Variable(tf.truncated_normal([240, 168], mean = mu, stddev = sigma))
    ...
    W_out = tf.Variable(tf.truncated_normal([168, n_classes], mean = mu, stddev = sigma))
...
def lenet_extra_layers(x, n_classes, grayscale=False):
    # Arguments used for tf.truncated_normal, randomly defines variables for the weights and biases for each layer
    mu = 0
    sigma = 0.1
    num_color_channles = 1 if grayscale else 3
    ...
    # Layer 4: Convolutional. Input = 12x12x16. Output = 10x10x52.
...
def lenet(x, n_classes, grayscale=False):
    # Arguments used for tf.truncated_normal, randomly defines variables for the weights and biases for each layer
    mu = 0
    sigma = 0.1
    num_color_channles = 1 if grayscale else 3
